...
// GloGeoConvOperations: Functions for converting between real-world values and the Godot GameEngine presentation.
...
public static class GloGeoConvOperations
{
    // --------------------------------------------------------------------------------------------
    // MARK: Distance Scaling
    // --------------------------------------------------------------------------------------------

    private static double RwEarthRadiusM = GloWorldConsts.EarthRadiusM; // Earth radius in meters
    private static double GeEarthRadius  = GloZeroOffset.GeEarthRadius; // Earth radius in Game Engine units
    ...
    // to the current presentation range GloZeroOffset.GeEarthRadius.
    ...
    // GloGeoConvOperations.RwToGeStruct(pos);
    public static Vector3 RwToGe(double radiusM, double latDegs, double lonDegs)
    {
        // Scale the radius
        radiusM = radiusM * GloZeroOffset.RwToGeDistanceMultiplier;

        // Convert the LLA to an XYZ - No offset for this function
        GloLLAPoint llap = new GloLLAPoint() { LatDegs = latDegs, LonDegs = lonDegs, RadiusM = radiusM };
    ...
    // GloGeoConvOperations.GeToRw(pos);
    public static GloLLAPoint GeToRw(Vector3 gePos)
    {
        ...
        double rwX = gePos.X * GloZeroOffset.GeToRwDistanceMultiplier;
        double rwY = gePos.Y * GloZeroOffset.GeToRwDistanceMultiplier;
        double rwZ = gePos.Z * GloZeroOffset.GeToRwDistanceMultiplier * -1;
    ...
    // Usage: Vector3 v3Pos = GloGeoConvOperations.RwToOffsetGe(pos);
    ...
    public static Vector3 RwToOffsetGe(GloLLPoint pos)
    {
        GloLLAPoint llaPos = new () { LatDegs = pos.LatDegs, LonDegs = pos.LonDegs, RadiusM = GloWorldConsts.EarthRadiusM };
        return GloZeroOffset.GeZeroPointOffset(llaPos.ToXYZ());
    }
    ...
    // Usage: GloPosV3 posV3 = GloGeoConvOperations.RwToGeStruct(Pos);
    public static GloPosV3 RwToGeStruct(GloLLAPoint pos)
    {
        // Define the position and associated up direction for the label
        GloLLAPoint posAbove = new () { LatDegs = pos.LatDegs,     LonDegs = pos.LonDegs, AltMslM = pos.AltMslM + GloZeroOffset.UpDistRwM};
        GloLLAPoint posNorth = new () { LatDegs = pos.LatDegs + 1, LonDegs = pos.LonDegs, AltMslM = pos.AltMslM};
    ...
    // Usage: GloEntityV3 platformV3 = GloGeoConvOperations.RealWorldToStruct(PlatformPos, PlatformCourse);
    ...
    public static GloEntityV3 RwToGeStruct(GloLLAPoint pos, GloCourse course)
    {
        ...
        GloRangeBearing aheadCourse = new GloRangeBearing() { RangeM = GloZeroOffset.AheadDistGeM, BearingDegs = course.HeadingDegs };
    ...
    public static GloEntityV3 RwToGeStruct(GloLLAPoint pos, double HeadingDegs)
    {
        ...
        // get the offset and ensure we have sufficient magnitude
        GloRangeBearing aheadCourse = new GloRangeBearing() { RangeM = GloZeroOffset.AheadDistGeM, BearingDegs = HeadingDegs };
        GloLLAPoint posAhead = pos.PlusRangeBearing(aheadCourse);
        Vector3 v3PosAhead   = GloZeroOffset.GeZeroPointOffset(posAhead.ToXYZ());
    ...
    // Usage: GloEntityV3 platformV3 = GloGeoConvOperations.RwToGeStruct(currPos, futurePos);
