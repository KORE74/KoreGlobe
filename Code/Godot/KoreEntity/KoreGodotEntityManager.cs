...
public partial class KoreGodotEntityManager : Node3D
{
    ...
    private void EntityEventCheck()
    {
        ...
        {
            string entName = createEvent.GetOrThrow(KoreEventRegister.KeyEntityName);

            if (!EntityExists(entName))
                AddEntity(entName);
        }
        ...
        {
            string entName = deleteEvent.GetOrThrow(KoreEventRegister.KeyEntityName);
    ...
    public KoreGodotEntity? GetEntity(string entityName)
    {
        foreach (Node3D currNode in EntityRootNode.GetChildren())
        {
            if (currNode.Name == entityName)
