...
public partial class GloElementContrail : Node3D
{
    List<GloLLAPoint> TrailPoints   = new List<GloLLAPoint>();
    ...
    public void AddTrailPoint(GloLLAPoint point)
    {
    ...
    public void AddTrailFromModel()
    {
        ...
        if (UseModel)
        {
            ...
            if (TimerModelTrail < GloCentralTime.RuntimeSecs)
            {
                TimerModelTrail = GloCentralTime.RuntimeSecs + 1f;

                GloLLAPoint? PlatformPos = GloAppFactory.Instance.EventDriver.GetPlatformPosition(ModelName);

                if (PlatformPos != null)
                    AddTrailPoint((GloLLAPoint)PlatformPos);
            }
        }
    }
    ...
    public void UpdateTrail()
    {
        ...
        if (numPoints > numNodes)
        {
            ...
            {
                MeshInstance3D meshInstance = GloPrimitiveFactory.CreateGodotSphere(Vector3.Zero, 0.025f, new Color(0.8f, 0.8f, 0.8f, 1.0f));
