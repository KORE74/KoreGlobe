...
public partial class GloZeroNodeMapTile : Node3D
{
    ...
    private GloLLAPoint     RwTileCenterLLA = GloLLAPoint.Zero; // Shortcut from the tilecode center
    private GloXYZPoint     RwTileCenterXYZ = GloXYZPoint.Zero; // Shortcut from the tilecode center
    private GloLLBox        RwTileLLBox     = GloLLBox.Zero; // Shortcut from the tilecode center

    // Parent/child tiles relationships
    public GloZeroNodeMapTile?      ParentTile = null;
    public List<GloZeroNodeMapTile> ChildTiles = new List<GloZeroNodeMapTile>();
    ...
    public GloFloat2DArray TileEleData  = new GloFloat2DArray();
    ...
    public GloUVBoxDropEdgeTile  UVBox = GloUVBoxDropEdgeTile.FullImage();
    ...
    private GloLineMesh3D  MeshInstanceW = new();
    ...
    public  GloRandomLoopList RandomLoopList = new GloRandomLoopList(10, 0.4f, 0.41f);
    ...
    private GloLatestHolder<GloZeroTileVisibilityStats> TileVisibilityStats = new GloLatestHolder<GloZeroTileVisibilityStats>();
    ...
    {
        TileVisibilityStats.LatestValue = new GloZeroTileVisibilityStats();
    ...
    public bool GrabbedActionCounter() => GloGodotFactory.Instance.ZeroNodeMapManager.ActionCounter.TryConsume();
    ...
    public void CreateChildTiles()
    {
        if (!TileCode.IsValid())
        {
            GloCentralLog.AddEntry("Invalid tile code for child tile creation.");
            return;
        }

        List<GloMapTileCode> childCodes = TileCode.ChildCodesList();

        foreach (GloMapTileCode currcode in childCodes)
            ChildTiles.Add(new GloZeroNodeMapTile(currcode));
    ...
    private void LocateTile()
    {
        ...
        Vector3 newPos = GloGeoConvOperations.RwToOffsetGe(RwTileCenterLLA);
