    ...
    public override void _Ready()
    {
        ...
        for (int lat = 80; lat >= -80; lat -= 10)
        {
            ...
            for (int lon = -180; lon < (180 - 5); lon += lonInc)
            {
                ...
                //Vector3 position      = GloGeoConvOperations.RealWorldToGodot(distance, lat, lon);
                //Vector3 northPosition = GloGeoConvOperations.RealWorldToGodot(distance, lat + 1, lon);
                //Vector3 eastPosition  = GloGeoConvOperations.RealWorldToGodot(distance, lat, lon + lonInc);
                //Vector3 southPosition = GloGeoConvOperations.RealWorldToGodot(distance, lat - latInc, lon);
