...
public partial class GloMeshBuilder
{
        ...
        int resolutionAz, int resolutionEl)
    {
        ...
        for (int y = 0; y <= resolutionEl; y++)
        {
            ...
            for (int x = 0; x <= resolutionAz; x++)
            {
                ...
                float currAzRads = (float)GloValueUtils.DegsToRads(currAzDegs);
                float currElRads = (float)GloValueUtils.DegsToRads(currElDegs);

                Vector3 outsidePoint = GloGodotGeometryOperations.Vector3FromPolar(Vector3.Zero, currAzRads, currElRads, distanceMax);
                Vector3 insidePoint  = GloGodotGeometryOperations.Vector3FromPolar(Vector3.Zero, currAzRads, currElRads, distanceMin);
        ...
        float insideRadius, float outsideRadius, int segments)
    {
        ...
        GloCentralLog.AddEntry($"azDeltaDegs:{azDeltaDegs} azimuthMaxDegs:{azimuthMaxDegs} azimuthMinDegs:{azimuthMinDegs} resolutionAz:{resolutionAz}");
