...
{
    class GloTcpServerClientConnection : GloCommonConnection
    {
        public bool running;
        ...
        private GloMsgSplitter IncomingMsgSplitter;

        private byte[] ReadBuffer = new byte[100 * 1024]; // 100 KB

        // ------------------------------------------------------------------------------------------------------------

        public GloTcpServerClientConnection()
        {
            running = false;
            ...
            IncomingMsgSplitter = new GloMsgSplitter(sentinelCharacter);
        ...
        void sendThreadFunc()
        {
            GloCentralLog.AddEntry($"Start ServerClient Send Thread {connectionDetailsString()}");
        ...
        {
            GloCentralLog.AddEntry($"Start ServerClient Receive Thread {connectionDetailsString()}");
            ...
            while (running)
            {
                ...
                if (client != null)
                {
                    if (!client.Connected)
                    {
                        GloCentralLog.AddEntry($"ServerClient Disconnected {connectionDetailsString()}");
                        client?.Close();
                        break;
                    }
                }
                ...
                {
                    GloCentralLog.AddEntry($"ServerClient Inactive {connectionDetailsString()} (10secs)");
                    client?.Close();
                    break;
                }
