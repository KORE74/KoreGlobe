...
// FssLLAPointOperations: A static class to hold operations on FssLLAPoint objects that are not part of its
// core responsibilites. This class is static, all operations are stateless and return a new object.

public static class FssLLPointOperations
{
    // Calculates the angle between two points on a sphere
    public static double AngleBetween(FssLLPoint fromPoint, FssLLPoint toPoint)
    {
        ...
        double deltaLon = FssValueUtils.AngleDiffRads(lon1, lon2);
    ...
    public static FssLLPoint RhumbLineInterpolation(FssLLPoint fromPoint, FssLLPoint toPoint, double fraction)
    {
        ...
        return new FssLLPoint() { LatDegs = latDegs, LonDegs = lonDegs };
    }

    // --------------------------------------------------------------------------------------------

    public static FssLLPoint GreatCircleInterpolation(FssLLPoint fromPoint, FssLLPoint toPoint, double fraction)
    {
        ...
        return new FssLLPoint() { LatDegs = newLatDegs, LonDegs = newLonDegs };
    }

    // --------------------------------------------------------------------------------------------

    // Generate a list of points that form a full circle path
    public static List<FssLLPoint> GreatCirclePointList(FssLLPoint fromPoint, FssLLPoint toPoint, int numPoints)
    {
        var points = new List<FssLLPoint>();
