...
public static class GloNumeric2DArrayIO<T> where T : struct, INumber<T>
{
    // --------------------------------------------------------------------------------------------
    // MARK: CSV Operations
    // --------------------------------------------------------------------------------------------

    public static string ToCSVString(GloNumeric2DArray<T> array, int decimalPlaces)
    {
    ...
    public static GloNumeric2DArray<T> FromCSVString(string csvString)
    {
        string[] lines = csvString.Trim().Split('\n');
        int rows = lines.Length;
        int cols = lines[0].Split(',').Length;
        GloNumeric2DArray<T> array = new GloNumeric2DArray<T>(cols, rows);
    ...
    public static void SaveToCSVFile(GloNumeric2DArray<T> array, string filePath, int decimalPlaces)
    {
        string csvString = ToCSVString(array, decimalPlaces);
        File.WriteAllText(filePath, csvString);
    }

    public static GloNumeric2DArray<T> LoadFromCSVFile(string filePath)
    {
    ...
    // MARK: Binary
    // --------------------------------------------------------------------------------------------
    public static void SaveToBinaryFile(GloNumeric2DArray<T> array, string filePath)
    {
    ...
    public static GloNumeric2DArray<T> LoadFromBinaryFile(string filePath)
    {
        using (BinaryReader reader = new BinaryReader(File.Open(filePath, FileMode.Open)))
        {
            int width = reader.ReadInt32();
            int height = reader.ReadInt32();
            GloNumeric2DArray<T> array = new GloNumeric2DArray<T>(width, height);
