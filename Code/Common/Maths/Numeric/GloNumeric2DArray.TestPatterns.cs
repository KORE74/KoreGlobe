...
public partial class GloNumeric2DArray<T> where T : struct, INumber<T>
{
    ...
    //
    // Usage example: GloNumeric2DArray<float> testPattern = GloNumeric2DArray<float>.LLATestPattern(4008, 2004, new GloLLBox(-180.0, -90.0, 180.0, 90.0));
    //
    // Note: Need to have the array upside down, so the top left corner is the top left corner of the map
    static public GloNumeric2DArray<T> LLATestPattern(int width, int height, GloLLBox llBox)
    {
        GloNumeric2DArray<T> retGrid = new GloNumeric2DArray<T>(width, height);
        ...
        GloNumeric2DArray<T> retGridrev = retGrid.ReverseRows();

        return retGridrev;
    }


    static public GloNumeric2DArray<T> LatTestPattern(int width, int height, GloLLBox llBox)
    {
        GloNumeric2DArray<T> retGrid = new GloNumeric2DArray<T>(width, height);
    ...
    static public GloNumeric2DArray<T> LonTestPattern(int width, int height, GloLLBox llBox)
    {
        GloNumeric2DArray<T> retGrid = new GloNumeric2DArray<T>(width, height);
