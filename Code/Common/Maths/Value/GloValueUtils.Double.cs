...
// GloValueUtils: A static class for common (double precision) math routines, useful as helper routines for higher-level functionality.
...
public static partial class GloValueUtils
{
    ...
    {
        if (Math.Abs(rangesize) < GloConsts.ArbitraryMinDouble)
    ...
    // GloValueUtils.LimitToRange(5, 0, 10) // Returns 5
    ...
    // Usage: GloValueUtils.AdjustWithinBounds(5, 10, 3) // Returns 8
    ...
    // Usage: GloValueUtils.EqualsWithinTolerance(val, val2, 0.3);

    public static bool EqualsWithinTolerance(double val, double matchval, double tolerance = GloConsts.ArbitraryMinDouble)
    {
        return Math.Abs(val - matchval) <= tolerance;
    }

    // Usage: GloValueUtils.IsZero(val)
    public static bool IsZero(double val, double tolerance = GloConsts.ArbitraryMinDouble)
    {
    ...
    // GloValueUtils.RandomInRange(1, 10) // Returns a random number between 1 and 10
