...
// GloValueUtils: A static class for common (float precision) math routines, useful as helper routines for higher-level functionality.
...
public static partial class GloValueUtils
{
    ...
    {
        if (Math.Abs(rangesize) < GloConsts.ArbitraryMinFloat)
    ...
    // Usage: GloValueUtils.Clamp(val, minval, maxval)
    ...
    public static bool EqualsWithinTolerance(float val, float matchval, float tolerance = GloConsts.ArbitraryMinFloat)
    {
