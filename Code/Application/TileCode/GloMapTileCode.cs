...
// GloMapTileCode: A tile code class, dealing solely with the definition of a hierachical grid format.

public struct GloLevelCodeElement
{
    public int LatIndex;
    public int LonIndex;
    ...
    {
        int latId = GloValueUtils.Clamp(LatIndex, 0, 19);
        int lonId = GloValueUtils.Clamp(LonIndex, 0, 19);
        return $"{GloMapTileCode.LetterLookup[latId]}{GloMapTileCode.LetterLookup[lonId]}";
    }
}

public class GloMapTileCode
{
    // --------------------------------------------------------------------------------------------
    // Constants
    // --------------------------------------------------------------------------------------------
    ...
    public List<GloLevelCodeElement> CodeList { get; private set; } = new();
    ...
    public Glo2DGridPos GridPos => new Glo2DGridPos(HorizCount, VertCount, HorizIndex, VertIndex);

    public GloLLBox LLBox => LLBoxForCode(this);
    ...
    public GloMapTileCode() {}

    public GloMapTileCode(int x, int y)
    {
        AddLevelCode(x, y, 0);
    }

    public GloMapTileCode(GloMapTileCode parentCode)
    {
        CodeList = new List<GloLevelCodeElement>(parentCode.CodeList);
    }

    public GloMapTileCode(string inStrCode)
    {
        GloMapTileCode newTileCode = TileCodeFromString(inStrCode);
        CodeList = new List<GloLevelCodeElement>(newTileCode.CodeList);
    }
    ...
    public static GloMapTileCode UndefinedTileCode()
    {
        return new GloMapTileCode();
    }

    public static GloMapTileCode Zero
    {
        get { return new GloMapTileCode(); }
    }
    ...
    public static GloMapTileCode? CodeToLvl(GloMapTileCode sourceCode, int toLvl)
    {
        if (sourceCode.IsValid() == false) return null;
        if (sourceCode.MapLvl > toLvl) return null;

        GloMapTileCode retCode = new GloMapTileCode();

        for (int i = 0; i < toLvl; i++)
        {
            GloLevelCodeElement l = sourceCode.CodeList[i];
    ...
    public void AddLevelCode(int x, int y, int level)
    {
        ...
        GloLevelCodeElement l = new GloLevelCodeElement() { LatIndex = y, LonIndex = x };
        CodeList.Add(l);
    }
    ...
    public void SetLevelCode(int x, int y, int level)
    {
        ...
        CodeList[level] = new GloLevelCodeElement() { LatIndex = y, LonIndex = x };
    }
    ...
    public static GloMapTileCode TileCodeFromString(string inStrCode)
    {
        // Create a blank tilecode.
        GloMapTileCode newTileCode = new();
        ...
        foreach (var part in parts)
        {
            if (part.Length != 2)
            {
                GloCentralLog.AddEntry($"Failed to parse: <{inStrCode}> // <{part}>");
                return newTileCode;
            }
        }
        ...
        foreach (var part in parts)
        {
            ...
            int firstIndex  = GloStringOperations.NumberForLetter(part[0]);
            int secondIndex = GloStringOperations.NumberForLetter(part[1]);
    ...
    public GloMapTileCode(double latDegs, double lonDegs, int mapLevel)
    {
    ...
    public GloMapTileCode ChildCode(int x, int y)
    {
        GloMapTileCode newCode = new GloMapTileCode(this);
    ...
    public GloMapTileCode ParentCode()
    {
        if (CodeList.Count == 0) return new GloMapTileCode();
    ...
    public List<GloMapTileCode> ChildCodesList()
    {
        List<GloMapTileCode> childCodes = new List<GloMapTileCode>();
    ...
    public Glo2DGridPos ChildPositionInParent()
    {
        return new Glo2DGridPos(HorizCount, VertCount, HorizIndex, VertIndex);
    }
    ...
    // GloMapTileCode.CodeForIndex(0, 0) => "AA"
    ...
    // Usage: GloLLBox tileBounds = GloMapTileCode.LLBoxForCode(tileCode);

    public static GloLLBox LLBoxForCode(GloMapTileCode tileCode)
    {
        ...
        int currLvl = 0;
        foreach (GloLevelCodeElement currCodeLevel in tileCode.CodeList)
        {
